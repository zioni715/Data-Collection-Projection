...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", default="", help="optional markdown output path"
    )
...
def main() -> None:
    ...
    output_lines.append("## 2) 시간대별 대표 행동(일자별 최상위 앱 다수결)\n")
    for hour in range(24):
        vote = Counter()
    ...
    output_lines.append("## 3) 앱 전체 사용량 Top\n")
    for app, sec in total_by_app.most_common(10):
        output_lines.append(f"- {app}: {sec//60}m")
    output_lines.append("")

    output_lines.append("## 4) 상세 제목(top)\n")
    for title, sec in titles.most_common(args.top_titles):
        output_lines.append(f"- {title}: {sec//60}m")
    output_lines.append("")

    output_lines.append("## 5) 해석 가이드\n")
    output_lines.append("- 시간대별 대표 행동은 단순 다수결로 계산됨(정밀 예측용이 아니라 패턴 참고용).")
    output_lines.append("- title은 window_title 기반이라 앱 내부 상세 정보는 제한적일 수 있음.")
    output_lines.append("")
